...
class App:
    ...
    def build_gui(self):
        ...
        self.button_lista_firm = tk.Button(self.root, text="Wybierz plik CSV z firmami", command=self.wybierz_lista_firm, font=("Arial", 12, "bold"))
        ...
        self.button_plik_csv = tk.Button(self.root, text="Wybierz plik CSV do importu", command=self.wybierz_plik_csv, font=("Arial", 12, "bold"))
        ...
        self.button_start = tk.Button(self.root, text="Uruchom przetwarzanie", command=self.controller.przetworz_dane, font=("Arial", 16, "bold"))
        self.button_start.pack(fill=tk.X, padx=10, pady=20)

    def wybierz_lista_firm(self):
        file_path = filedialog.askopenfilename(title="Wybierz plik CSV z firmami", filetypes=[("Pliki CSV", "*.csv")])
        if file_path:
            self.controller.set_lista_firm_path(file_path)
            self.label_lista_firm.config(text=str(file_path))

    def wybierz_plik_csv(self):
        file_path = filedialog.askopenfilename(title="Wybierz plik CSV do importu", filetypes=[("Pliki CSV", "*.csv")])
        if file_path:
